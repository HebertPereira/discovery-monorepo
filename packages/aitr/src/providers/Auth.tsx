...
export function IdeaProvider({ children }: IdeaProviderProps) {
  ...
  const [initialListOfIdeas, setInitialListOfIdeas] = useState<
    ListOfIdeasProps[]
  >([]);
    ...
    api
      ...
      .catch((error: AxiosError) => {
        if (error.response?.status === 500)
          return toast.error('Erro interno de servidor!');
  ...
  async function createIdea(ideaInput: IdeaInputProps) {
    ...
    setListOfIdeas([...listOfIdeas, ideas]);
  }
  ...
  function searchIdea(searchValue: string, filterOnlyTags: boolean) {
    ...
    const searchAll = initialListOfIdeas.filter(
